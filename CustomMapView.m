...
-(void)customizeMap:(NSMutableArray*)arrayWellData
{
    ...
    for(int i=0;i<[arrayWellData count];i++)
    {
        ...
        ////NSLog(@"%@",arrayWellData);
        location.latitude = [[[arrayWellData objectAtIndex:i] objectForKey:@"Lat"] floatValue];
        location.longitude = [[[arrayWellData objectAtIndex:i] objectForKey:@"Long"] floatValue];
